...
func TestNotFoundUsesErrorContract(t *testing.T) {
	gin.SetMode(gin.TestMode)
	recorder := httptest.NewRecorder()
	NewRouter(handler.NewAuthHandler(nil), nil, nil).ServeHTTP(recorder, httptest.NewRequest(http.MethodGet, "/missing", nil))
...
func TestCurrentUserRequiresAuthentication(t *testing.T) {
	gin.SetMode(gin.TestMode)
	recorder := httptest.NewRecorder()
	NewRouter(handler.NewAuthHandler(nil), nil, nil).ServeHTTP(recorder, httptest.NewRequest(http.MethodGet, "/api/v1/me", nil))
